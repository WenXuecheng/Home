<template>
  <div ref="containerRef" class="beams-container"></div>
</template>
...
const initThreeJS = () => {
  ...
  group.rotation.set(degToRad(props.tilt), 0, degToRad(props.rotation));
  group.add(beamMesh);
  scene.add(group);
  ...
  const updateGroupScale = () => {
    ...
    const baseHeight = Math.max(props.beamHeight, 1);
    const totalWidth = Math.max(
      props.beamNumber * props.beamWidth + Math.max(props.beamNumber - 1, 0) * props.beamSpacing,
      1
    );
